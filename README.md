...
dns端口就用53,因为上面我们已经把Dnsmasq的dns功能关闭了，所以这里adguard可以占用53端口。 web访问端口不要用80,还是改回3000。

最后配置好管理账号密码以后 进入adg的管理面板
...
创建一个自动更新脚本 比如
```sh
# 这是你打算储存脚本文件的路径,先确保目录存在
nano /overlay/data/adguard_upstream_dns_file_update.sh
```
 内容如下
```sh
#!/bin/sh

# 定义路径变量 这是你打算储存规则文件的路径,先确保目录存在
DNS_FILE_PATH="/overlay/data/adguard_upstream_dns_file.txt"
rm -rf "$DNS_FILE_PATH" # 删除旧文件
# 下载新规则文件
wget "https://你的githubProxy代理站地址/https://raw.githubusercontent.com/joyanhui/adguardhome-rules/refs/heads/release_file/ADG_chinaDirect_WinUpdate_Gfw.txt" -O "$DNS_FILE_PATH"
# 替换为你的doh域名和后缀
sed -i 's/d-o-h.you-cf-domain.com/你的doh域名部分/g' "$DNS_FILE_PATH"
sed -i 's/your-suffix/你的doh后缀部分/g' "$DNS_FILE_PATH"
# 重启adguardhome
